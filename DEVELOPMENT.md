...
The C/C++ extension automatically uses `.clang-format`. Format on save is enabled by default in `.vscode/settings.json`:
...
2. **Format code**: `clang-format -i src/main.c`
3. **Test locally**: `pio run`
...
- **Format on save** in VS Code to avoid formatting commits
- **Run pre-commit locally** before pushing to catch issues early
...
- Install in container: `apt-get install clang-format`
- Or use system clang-format: `pre-commit run --hook-stage manual`
